...
/**
 * The ControlCenter class represents the central control center of a rental system.
...
public class ControlCenter implements Observer, Timer {
     ...
     * If the instance does not exist, it creates a new instance with the given lists of rental stations and repairers.
     * 
    ...
    /**
     * Redistributes the stations by initializing the colors, performing the redistribution,
    ...
    public void vehicleOutOfService(Vehicle vehicle) {
        ...
        if (found) {
            ...
            System.out.println("No repairer available, wait");
        }
    }
    ...
    /**
     * Notifies the control center that a stolen vehicle has been recovered and is back in possession.
     * 
     ...
     * This method checks if any rental station is empty or full, and if so, it sets the 'found' flag to true.
     * If the 'found' flag is true, it increments the redistribution counter by 1, otherwise it resets the counter to 0.
     * If the redistribution counter reaches the threshold value, it triggers the redistribution process.
     * Finally, it increments the rental duration of all rented vehicles and displays the current state of the rental stations.
     */
    public void tick() {
        boolean found = false;
